...
PYBIND11_MODULE(_tram_bindings, m) {
    ...
    using namespace deeptime::markov::tram;
    {
        ...
        py::class_<TRAM<TRAMInput<double>>>(tramMod, "TRAM")
                ...
                .def("estimate", &TRAM<TRAMInput<double>>::estimate,
                ...
                .def_property_readonly("transition_matrices", &TRAM<TRAMInput<double>>::transitionMatrices)
                .def_property_readonly("biased_conf_energies", &TRAM<TRAMInput<double>>::biasedConfEnergies)
                .def_property_readonly("modified_state_counts_log",
                                       &TRAM<TRAMInput<double>>::modifiedStateCountsLog)
                .def_property_readonly("lagrangian_mult_log", &TRAM<TRAMInput<double>>::lagrangianMultLog)
                .def_property_readonly("therm_state_energies", &TRAM<TRAMInput<double>>::thermStateEnergies)
                .def_property_readonly("markov_state_energies", &TRAM<TRAMInput<double>>::markovStateEnergies);

        py::class_<TRAMInput<double>, std::shared_ptr<TRAMInput<double>>>(tramMod, "TRAMInput").def(
                py::init<deeptime::np_array_nfc<int> &&, deeptime::np_array_nfc<int> &&, BiasMatrices<double>, BiasMatrix<double> &&>(),
                "state_counts"_a, "transition_counts"_a, "bias_matrix"_a, "bias_coefficients"_a);
